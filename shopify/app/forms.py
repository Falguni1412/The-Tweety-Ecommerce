...
class AddressForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'locality': forms.TextInput(attrs={'placeholder': 'Street, Building', 'class': 'form-control'}),
            'city': forms.TextInput(attrs={'placeholder': 'City', 'class': 'form-control'}),
            'pincode': forms.NumberInput(attrs={'placeholder': '6-digit PIN', 'class': 'form-control'}),
            'state': forms.TextInput(attrs={'placeholder': 'State', 'class': 'form-control'}),
            'phone': forms.TextInput(attrs={'placeholder': 'Mobile', 'class': 'form-control'}),
...
class ProductFilterForm(forms.Form):
    ...
    min_rating = forms.IntegerField(required=False, min_value=1, max_value=5, widget=forms.NumberInput(attrs={
        'placeholder': '4+', 'class': 'form-control'
    }))
    in_stock_only = forms.BooleanField(required=False, widget=forms.CheckboxInput(attrs={
        'class': 'form-check-input'
    }))
    free_delivery = forms.BooleanField(required=False, widget=forms.CheckboxInput(attrs={
        'class': 'form-check-input'
    }))
    sort = forms.ChoiceField(choices=SORT_CHOICES, required=False, widget=forms.Select(attrs={
        'class': 'form-select'
    }))
...
class ProductForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            'selling_price', 'discounted_price', 'unit',
            'stock', 'product_image', 'return_days', 'tags',
            'is_featured', 'is_bestseller', 'is_new',
            'free_delivery', 'cash_on_delivery',
        ]
...
class OrderForm(forms.Form):
    address = forms.ModelChoiceField(queryset=Address.objects.none(), empty_label="Select Address")
...
class CardPaymentForm(forms.Form):
    ...
    card_holder = forms.CharField(
        max_length=100,
        widget=forms.TextInput(attrs={'placeholder': 'Name on Card', 'class': 'form-control'})
    )
    expiry_date = forms.CharField(
        max_length=7,
        widget=forms.TextInput(attrs={
            'placeholder': 'MM/YYYY', 'class': 'form-control', 'maxlength': '7'
        })
    )
    cvv = forms.CharField(
        max_length=4,
        widget=forms.PasswordInput(attrs={
            'placeholder': 'CVV', 'class': 'form-control', 'maxlength': '4'
        })
    )

    def clean_card_number(self):
        cn = self.cleaned_data.get('card_number', '').replace(' ', '').replace('-', '')
...
class UPIForm(forms.Form):
    upi_id = forms.CharField(
        max_length=100,
        widget=forms.TextInput(attrs={'placeholder': 'yourname@bank', 'class': 'form-control'}),
        help_text='Example: yourname@upi'
    )

    def clean_upi_id(self):
        upi = self.cleaned_data.get('upi_id', '').strip()
...
class NetbankingForm(forms.Form):
    ...
    bank = forms.ChoiceField(choices=BANK_CHOICES, widget=forms.Select(attrs={'class': 'form-select'}))
...
class WalletForm(forms.Form):
    ...
    wallet = forms.ChoiceField(choices=WALLET_CHOICES, widget=forms.Select(attrs={'class': 'form-select'}))
    mobile = forms.CharField(max_length=10, widget=forms.TextInput(attrs={
        'placeholder': 'Registered mobile', 'class': 'form-control'
    }))
...
class ReviewForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'title': forms.TextInput(attrs={'placeholder': 'Summarize your review', 'class': 'form-control'}),
            'comment': forms.Textarea(attrs={
                'placeholder': 'Share your experience...', 'rows': 4, 'class': 'form-control'
            }),
        }
...
class ReturnRequestForm(forms.ModelForm):
    class Meta:
        model = ReturnRequest
        fields = ['reason', 'reason_detail', 'refund_method', 'bank_account']
        widgets = {
            ...
            'bank_account': forms.TextInput(attrs={
                'class': 'form-control', 'placeholder': 'For refund via bank account'
            }),
        }
